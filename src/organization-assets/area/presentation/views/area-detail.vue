...
const store = useAreaStore(); const siteStore = useSiteStore(); const assetStore = useAssetStore()
const area = computed(() => store.getById(route.params.id))
const siteName = computed(() => siteStore.sites.find(s => s.id === area.value?.sedeId)?.nombre ?? '-')
const assets = computed(() => assetStore.assets.filter(a => a.areaId === parseInt(route.params.id)))
...
onMounted(() => { if (!store.loaded) store.fetchAll(); if (!siteStore.loaded) siteStore.fetchAll(); if (!assetStore.loaded) assetStore.fetchAll() })
</script>
...
<template>
  <div v-if="area">
    <div class="rg-page-header">
      ...
      <pv-button :label="t('common.edit')" icon="pi pi-pencil" size="small" @click="router.push(`/organization-assets/area/${area.id}/edit`)" />
    </div>
    <div style="display:grid;grid-template-columns:1fr 1fr;gap:12px">
      <div class="rg-card">
        ...
        <div class="rg-detail-row"><span class="rg-detail-label">{{ t('area.sede') }}</span><span class="rg-detail-value">{{ siteName }}</span></div>
      ...
      <div class="rg-card">
        ...
        <div v-for="a in assets" :key="a.id" class="rg-detail-row" style="cursor:pointer" @click="router.push(`/organization-assets/asset/${a.id}`)">
        ...
        <div v-if="!assets.length" style="color:var(--rg-text-muted);font-size:0.82rem">{{ t('common.noData') }}</div>
      </div>
    </div>
  </div>
</template>
