...
<template>
  <div>
    <div class="rg-page-header">
      ...
      <pv-button :label="t('activo.new')" icon="pi pi-plus" size="small" @click="router.push('/organization-assets/asset/new')" />
    </div>
    <div class="rg-table-wrap">
      <pv-data-table :value="store.assets" :loading="!store.loaded" striped-rows size="small" :rows="10" paginator>
        ...
        <pv-column :header="t('common.actions')" style="width:110px">
          <template #body="{ data }">
            <pv-button icon="pi pi-eye" size="small" text rounded @click="router.push(`/organization-assets/asset/${data.id}`)" />
            <pv-button icon="pi pi-pencil" size="small" text rounded severity="secondary" @click="router.push(`/organization-assets/asset/${data.id}/edit`)" />
