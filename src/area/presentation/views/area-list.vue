...
<template>
  <div>
    <div class="rg-page-header">
      ...
      <pv-button :label="t('area.new')" icon="pi pi-plus" size="small" @click="router.push('/area/new')" />
    </div>
    <div class="rg-table-wrap">
      <pv-data-table :value="store.areas" :loading="!store.loaded" striped-rows size="small" :rows="10" paginator>
        ...
        <pv-column :header="t('common.actions')" style="width:110px">
          <template #body="{ data }">
            <pv-button icon="pi pi-eye" size="small" text rounded @click="router.push(`/area/${data.id}`)" />
            <pv-button icon="pi pi-pencil" size="small" text rounded severity="secondary" @click="router.push(`/area/${data.id}/edit`)" />
