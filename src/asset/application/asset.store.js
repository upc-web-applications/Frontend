...
import { ref } from 'vue'
import { AssetApi } from '@/asset/infrastructure/asset-api.js'
import { AssetAssembler } from '@/asset/infrastructure/asset.assembler.js'
/**
...
export const useAssetStore = defineStore('asset', () => {
    ...
    function fetchAll() { api.getAssets().then(r => { assets.value = AssetAssembler.toEntitiesFromResponse(r); loaded.value = true }).catch(e => errors.value.push(e)) }
    ...
    function update(asset) { return api.updateAsset(asset).then(r => { const u = AssetAssembler.toEntityFromResource(r.data); const i = assets.value.findIndex(a => a.id === u.id); if (i !== -1) assets.value[i] = u }).catch(e => errors.value.push(e)) }
