...
const SESSION_STORAGE_KEY = 'riskguard-current-session'

function readStoredEntity(key, Entity) {
  try {
    const resource = JSON.parse(localStorage.getItem(key))
    return resource ? new Entity(resource) : null
  } catch {
...
const useIdentityAccessStore = defineStore('identity-access', () => {
  ...
  const roles = ref([])
  const errors = ref([])
  const currentUser = ref(readStoredEntity(USER_STORAGE_KEY, User))
  const currentSession = ref(readStoredEntity(SESSION_STORAGE_KEY, Session))
  ...
  function authenticate(email, password) {
    ...
    return identityAccessApi.updateUser(user).then(() => {
      ...
      localStorage.setItem(SESSION_STORAGE_KEY, JSON.stringify(currentSession.value))
