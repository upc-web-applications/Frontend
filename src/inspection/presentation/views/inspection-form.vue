...
const form = ref({ tipoIncidente:'', sedeId:null, areaId:null, activoId:null, nivelUrgencia:'', descripcion:'', estado:'Pendiente', fotoUrl:null, operarioId:null, ticket:'', fechaReporte:'', fechaActualizacion:'', accionCorrectiva:null })
...
const onFileChange = (e) => {
    ...
    if (!file) return
    if (!['image/jpeg', 'image/png'].includes(file.type)) { photoError.value = 'Solo se permiten imagenes JPG o PNG'; return }
...
const submit = async () => {
    const missingRequired = !form.value.tipoIncidente || !form.value.sedeId || !form.value.areaId || !form.value.nivelUrgencia || !form.value.descripcion.trim()
    if (missingRequired || (form.value.tipoIncidente === 'Otro' && !otroTipo.value.trim())) {
        toast.add({ severity: 'warn', summary: 'Complete los campos obligatorios antes de enviar', life: 4000 })
        return
    }
    ...
    try {
        ...
        const result = await store.add({
            ...form.value,
            tipoIncidente: form.value.tipoIncidente === 'Otro' ? otroTipo.value.trim() : form.value.tipoIncidente,
    ...
    <div class="inspection-form-grid">
      ...
      <div class="rg-card" style="display:flex;flex-direction:column;gap:12px">
        ...
        <div v-if="form.tipoIncidente === 'Otro'" class="rg-form-field">
          <label class="rg-label">Especifique el tipo *</label>
          <pv-input-text v-model="otroTipo" maxlength="80" size="small" style="width:100%" />
        </div>
        <div class="rg-form-field">
