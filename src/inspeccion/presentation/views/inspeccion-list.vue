...
const store = useInspeccionStore(); const areaStore = useAreaStore(); const sedeStore = useSedeStore()
...
const pendientes  = computed(() => store.inspecciones.filter(i => i.estado === 'Pendiente').length)
const enProgreso  = computed(() => store.inspecciones.filter(i => i.estado === 'En Progreso').length)
const resueltos   = computed(() => store.inspecciones.filter(i => i.estado === 'Resuelto').length)
...
<template>
  <div>
    <div class="rg-page-header">
      ...
      <pv-button :label="t('inspeccion.new')" icon="pi pi-plus" size="small" @click="router.push('/inspeccion/new')" />
    </div>
    ...
    <div class="rg-table-wrap">
      <pv-data-table :value="store.inspecciones" :loading="!store.loaded" striped-rows size="small" :rows="10" paginator>
        ...
        <pv-column :header="t('common.actions')" style="width:90px">
          <template #body="{ data }">
            <pv-button icon="pi pi-eye" size="small" text rounded @click="router.push(`/inspeccion/${data.id}`)" />
