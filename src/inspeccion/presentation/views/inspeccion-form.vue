...
const activoOptions = computed(() => activoStore.activos.filter(a => a.areaId === form.value.areaId && a.estado === 'Activo').map(a => ({ label: `${a.codigo} — ${a.nombre}`, value: a.id })))
...
const submit = async () => {
    ...
    try {
        ...
        router.push(`/inspeccion/${result.id}`)
    } catch {
    ...
    <div style="display:grid;grid-template-columns:1fr 1fr;gap:12px">
      ...
      <div class="rg-card" style="display:flex;flex-direction:column;gap:12px">
        ...
        <div class="rg-form-field">
          ...
          <pv-select v-model="form.activoId" :options="activoOptions" option-label="label" option-value="value" :disabled="!form.areaId" show-clear size="small" style="width:100%" />
        </div>
      </div>
