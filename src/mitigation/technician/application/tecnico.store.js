...
import { ref } from 'vue'
import { TecnicoApi } from '@/mitigation/technician/infrastructure/tecnico-api.js'
import { TecnicoAssembler } from '@/mitigation/technician/infrastructure/tecnico.assembler.js'
...
export const useTecnicoStore = defineStore('tecnico', () => {
    ...
    function fetchAll() { errors.value = []; return api.getTecnicos().then(r => { tecnicos.value = TecnicoAssembler.toEntitiesFromResponse(r); loaded.value = true }).catch(e => { errors.value.push(e); return Promise.reject(e) }) }
    function getById(id) { return tecnicos.value.find(t => t.id === id) }
