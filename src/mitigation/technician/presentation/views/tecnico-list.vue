...
const confirmDelete = (tecnico) => {
    confirm.require({
        message: 'Eliminar este tecnico?', header: t('common.confirm'), icon: 'pi pi-exclamation-triangle', acceptClass: 'p-button-danger',
        accept: () => store.remove(tecnico.id).then(() => toast.add({ severity: 'success', summary: 'Tecnico eliminado', life: 3000 })).catch(() => toast.add({ severity: 'error', summary: t('common.error'), detail: 'No se pudo eliminar el tecnico', life: 5000 }))
    })
}
</script>
...
<template>
  <div>
    <div class="rg-page-header">
      ...
      <pv-button :label="t('tecnico.new')" icon="pi pi-plus" size="small" @click="router.push('/mitigation/technicians/new')" />
    </div>
    <div class="rg-table-wrap">
      <pv-data-table :value="store.tecnicos" :loading="!store.loaded" striped-rows size="small" :rows="10" paginator>
        ...
        <pv-column :header="t('common.actions')" style="width:110px">
          <template #body="{ data }">
            <pv-button icon="pi pi-eye" size="small" text rounded @click="router.push(`/mitigation/technicians/${data.id}`)" />
            <pv-button icon="pi pi-pencil" size="small" text rounded severity="secondary" @click="router.push(`/mitigation/technicians/${data.id}/edit`)" />
