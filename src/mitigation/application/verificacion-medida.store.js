...
export const useVerificacionMedidaStore = defineStore('verificacionMedida', () => {
    ...
    function fetchAll() { api.getVerificaciones().then(r => { verificaciones.value = VerificacionMedidaAssembler.toEntitiesFromResponse(r); loaded.value = true }).catch(e => errors.value.push(e)) }
    function getById(id) { return verificaciones.value.find(v => v.id === parseInt(id)) }
    function getByTicketId(tId) { return verificaciones.value.filter(v => v.ticketId === parseInt(tId)) }
