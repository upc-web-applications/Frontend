...
export const useNotificacionCriticaStore = defineStore('notificacionCritica', () => {
    ...
    function fetchAll() { api.getNotificaciones().then(r => { notificaciones.value = NotificacionCriticaAssembler.toEntitiesFromResponse(r); loaded.value = true }).catch(e => errors.value.push(e)) }
    function getById(id) { return notificaciones.value.find(n => n.id === parseInt(id)) }
    function getByTicketId(tId) { return notificaciones.value.filter(n => n.ticketId === parseInt(tId)) }
