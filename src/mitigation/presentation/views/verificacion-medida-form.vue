...
const submit = async () => {
    ...
    try {
        ...
        if (created && created.veredicto === 'Aprobado' && tk) {
            ...
            await historialStore.add({ ticketId: tk.id, evento: 'Ticket cerrado', usuarioId: 1, usuarioNombre: form.value.supervisorNombre || 'Supervisor', detalles: `Mitigación aprobada: ${form.value.comentarioJustificacion || 'Sin comentarios'}`, fecha: hoy })
            ...
            toast.add({ severity: 'success', summary: 'Ticket cerrado y mitigación aprobada', life: 3000 })
        } else if (created && created.veredicto === 'Rechazado' && tk) {
            await ticketStore.update({ ...tk, estado: 'Escalado' })
            await historialStore.add({ ticketId: tk.id, evento: 'Ticket escalado', usuarioId: 1, usuarioNombre: form.value.supervisorNombre || 'Supervisor', detalles: `Mitigación rechazada: ${form.value.comentarioJustificacion || 'Sin comentarios'}`, fecha: hoy })
            await notifStore.add({ ticketId: tk.id, supervisorId: 1, supervisorNombre: form.value.supervisorNombre || 'Supervisor', mensaje: `Ticket #${tk.id} escalado - mitigación rechazada en ${tk.sector}`, enviada: true, fechaEnvio: hoy })
            const mit = mitigationStore.getByTicketId(tk.id)
